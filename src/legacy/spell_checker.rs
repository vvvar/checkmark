...
pub async fn check(path: &str) -> Result<Vec<Issue>> {
    ...
    for line in String::from(include_str!(
        "spell_dictionary/frequency_dictionary_en_82_765.txt"
    ))
    .lines()
    {
    ...
    for line in String::from(include_str!(
        "spell_dictionary/frequency_bigramdictionary_en_243_342.txt"
    ))
    .lines()
    {
        ...
        for word in text_only.split_ascii_whitespace() {
            ...
            if !suggestions.is_empty() && !suggestions.first().unwrap().term.eq(&word) {
                let mut issue_suggestions = vec![format!(
                    ...
                    &word.red(),
                    suggestions.first().unwrap().term.green()
                )];
                    ...
                    && !compound_suggestions.first().unwrap().term.eq(&text_only)
                {
                    issue_suggestions.push(format!(
                        ...
                        text_only.red(),
                        "+ ".green(),
                        compound_suggestions.first().unwrap().term.green()
                    ));
                }
