...
fn invalid_file_error_to_issues(file: &mut common::MarkDownFile, unreachable_file_path: &str) {
    log::debug!("Handling unreachable file error: {:#?}", &unreachable_file_path);
    ...
    let file_path_in_document = match unreachable_file_path.replace(&document_folder_path, "").strip_prefix("/") {
        Some(stripped) => stripped.to_string(),
        None => unreachable_file_path.replace(&document_folder_path, "")
    };
    log::debug!("Path as it is specified in document: {:#?}", &file_path_in_document);
...
async fn collect_links(path: &str, ignored_uri_wildcards: &Vec<String>) -> lychee_lib::Result<std::collections::HashMap<String, lychee_lib::Request>> {
    log::debug!("Collecting links in file: {:#?}, ignoring these links: {:#?}", &path, &ignored_uri_wildcards);
        ...
        match lychee_lib::check(request).await.unwrap().status() {
            ...
            lychee_lib::Status::Error(error_kind) => match error_kind {
                ...
                lychee_lib::ErrorKind::ReadStdinInput(error) => {
                    log::debug!("{:#?} request error while reading stdin as input, error:\n{:#?}", &uri, &error);
                },
                // Errors which can occur when attempting to interpret a sequence of u8 as a string
                lychee_lib::ErrorKind::Utf8(error) => {
                    log::debug!("{:#?} request error while interpret a sequence of u8, error:\n{:#?}", &uri, &error);
                },
                // The Github client required for making requests cannot be created
                lychee_lib::ErrorKind::BuildGithubClient(error) => {
                    log::debug!("{:#?} request error cant create github client, error:\n{:#?}", &uri, &error);
                },
                // Invalid Github URL
                lychee_lib::ErrorKind::InvalidGithubUrl(invalid_github_url) => {
                    log::debug!("{:#?} request error invalid GitHub URL: {:#?}", &uri, &invalid_github_url);
                },
                ...
                lychee_lib::ErrorKind::ParseUrl(error, url) => {
                    log::debug!("{:#?} request error cant parse URL, error:\n{:#?}", &url, &error);
                },
                ...
                lychee_lib::ErrorKind::UnreachableEmailAddress(uri, email) => {
                    log::debug!("{:#?} request error unreachable e-mail, email:{:#?}", &uri, &email);
                },
                ...
                lychee_lib::ErrorKind::InvalidGlobPattern(glob_error) => {
                    log::debug!("{:#?} request error given glob pattern is not valid, error:\n{:#?}", &uri, &glob_error);
                },
                ...
                lychee_lib::ErrorKind::InvalidUrlHost => {
                    log::debug!("{:#?} request error URL with an invalid host was found", &uri);
                },
            ...
            lychee_lib::Status::Timeout(status_code) => {
                log::debug!("{:#?} request timeout, status code: {:#?}", &uri, &status_code);
            },
            // Got redirected to different resource
            lychee_lib::Status::Redirected(status_code) => {
                log::debug!("{:#?} request redirected with status code :{:#?}", &uri, &status_code);
            },
            // The given status code is not known by lychee
            lychee_lib::Status::UnknownStatusCode(status_code) => {
                log::debug!("{:#?} request error unknown status code: {:#?}", &uri, &status_code);
            },
            ...
            lychee_lib::Status::Unsupported(error_kind) => {
                log::debug!("{:#?} request type is currently not supported, error:\n{:#?}", &uri, &error_kind);
            },
            // Cached request status from previous run
            lychee_lib::Status::Cached(cache_status) => {
                log::debug!("{:#?} request cached, cache status: {:#?}", &uri, &cache_status);
            }
        };
    }
}