...
pub async fn check(path: &String, api_key: &String) -> Result<Vec<Issue>, Box<dyn std::error::Error>> {
    ...
    for (num_line, line) in original.lines().enumerate() {
        ...
        if !text_only.is_empty() {
            ...
            let resp: serde_json::Value = client.post("https://api.sapling.ai/api/v1/edits")
                .json(&map).send().await?
                .json().await?;
            match resp["edits"].as_array() {
                Some(edits) => {
                    for edit in edits {
                        let general_error_type = edit["general_error_type"].as_str().expect("Sapling haven't sent correction type");
                        let sentence = edit["sentence"].as_str().expect("Sapling haven't sent a sentence to correct");
                        let replacement = edit["replacement"].as_str().expect("Sapling haven't sent a replacement");
                        ...
                        let problematic_part_of_phrase = &sentence[replacement_start..replacement_end];
                        issues.push(Issue {
